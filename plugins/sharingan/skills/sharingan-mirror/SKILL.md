...
Siga as checagens de permissao e exclusoes da skill `sharingan`. Use o subagente `danzo` para a copia quando o destino estiver definido. Depois de espelhar, ofereca `sharingan-brand` e `sharingan-validate`. O slash command desta skill e `/sharingan-clone`, nao `/sharingan-mirror`.
